...
const HomeData = async () => {
  ...
  return (
    <div className="py-6 space-y-8">
      {/* Stats*/}
      <div className="grid grid-cols-2 sm:grid-cols-4 md:items-stretch justify-between lg:items-center gap-4">
        ...
        <div className="p-8 rounded-md bg-white flex flex-col gap-3 justify-between items-center text-center w-full">
          <h2 className="text-5xl font-medium">{needAttention.length}</h2>
          ...
          {friends.map((friend) => (
            <Link key={friend.id} href={`/friend/${friend.id}`}>
              <div className="bg-white shadow-sm rounded-xl p-4 flex flex-col justify-between items-center gap-2">
                ...
                />
                <h3 className="text-md sm:text-xl font-semibold text-[#1F2937]">
                  {friend.name}
                </h3>
                <div className="flex flex-col justify-between items-center gap-1">
                  ...
                  <div className="flex justify-center gap-1">
                    {friend.tags.map((tag, ind) => (
                      <div
                        key={ind}
                        className="badge rounded-3xl badge-info text-sm sm:text-lg text-white"
                      >
                        {tag}
                      </div>
                    ))}
                  ...
                  <div
                    className={`badge rounded-3xl text-sm sm:text-lg text-white ${friend.status === "on-track" ? "badge-primary" : friend.status === "overdue" ? "badge-error" : friend.status === "almost due" ? "badge-warning" : ""}`}
                  >
                    {friend.status}
                  </div>
                </div>
